...
On the direct path the funds never enter Fermah's control. The signed authorisation targets the application contract directly, and the invocation tree includes the token transfer as a sub-invocation authorised by the user's signature. The application contract moves the user's USDC to itself and credits the output to the address in the authorisation entry. The gateway's own accounting is not in the path: it calls neither `credit` nor `charge_batch` and writes no balance row. A failed relay leaves nothing mis-moved, because nothing moved.
...
A Soroban authorisation entry covers the contract address, the function name, and every argument. For a swap, the minimum output to accept and the sponsor are all arguments to the application contract's function call. They are inside the signed invocation tree. A relayer that changes any of them produces a transaction whose authorisation entry no longer matches the user's signature — the Soroban runtime rejects it.
